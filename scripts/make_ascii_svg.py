...
# ---- identity (shown in the fake terminal chrome) --------------------------
USERNAME: str = "arijit"
DISPLAY_NAME: str = "Arijit Das"
...
SRC: Path = Path(sys.argv[1]) if len(sys.argv) > 1 else ASSETS_DIR / "source-prepped.png"
OUT: Path = Path(sys.argv[2]) if len(sys.argv) > 2 else ASSETS_DIR / f"{USERNAME}-ascii.svg"

COLS: int = 100
ROWS: int = 53
CELL_W: int = 8
CELL_H: int = 15
RAMP: str = " .`:-=+*cs#%@"  # bright(sparse) -> dark(dense); leading space clears bg

# the prepped image already has bg removed + CLAHE local contrast, so only
# light global tuning is needed here.
CONTRAST: float = 1.05
BRIGHTNESS: float = 1.0
GAMMA: float = 1.18          # >1 brightens mids -> face lands in sparser chars
SHARPEN: bool = False
WHITE_FLOOR: float = 0.80    # luminance above this is forced to blank (space)
...
BG2: str = "#111722"
FRAME: str = "#30363d"
TITLE_TEXT: str = "#7d8590"
INK: str = "#c9d1d9"      # the single ascii color (matches Andrew6rant)
CURSOR: str = "#c9d1d9"

# ---- reveal timing (one-shot; a cursor rasters top -> bottom) -------------
ROW_DUR: float = 0.11
STAGGER: float = 0.11       # == ROW_DUR -> a single cursor sweeping down
